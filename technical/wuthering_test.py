...
# --- CONFIGURATION ---
URL = "https://www.gutenberg.org/files/768/768-0.txt"  # Wuthering Heights
OUTPUT_FILE = "agency_phase_transition_heathcliff.png"
...
print("--- DOWNLOADING WUTHERING HEIGHTS ---")
...
clean_text = full_text[start_idx:end_idx]
...
for i, chapter in enumerate(chapters):
    if len(chapter.strip()) < 1000:
        continue  # Skip empty/short sections
...
y = np.linspace(5, 25, 100)  # Extended Y for high obsession scores
X, Y = np.meshgrid(x, y)
Z = np.sin(X) * Y - (X * 2)
...
sns.scatterplot(
    ...
    marker="D",
    edgecolor="black",
    label="Wuthering Heights (Human Obsession)"
)

# 3. Annotate
for i in range(df.shape[0]):
    # Annotate only high outliers
    if df.Agency.iloc[i] > 18 or df.Agency.iloc[i] < 12:
        plt.text(df.Entropy.iloc[i], df.Agency.iloc[i] + 0.2, df.Label.iloc[i], fontsize=9, fontweight='bold')

plt.title("Thermodynamics of Obsession: Wuthering Heights", fontsize=16, fontweight='bold')
plt.xlabel("Shannon Entropy (H)", fontsize=12)
plt.ylabel("Agency Index (Optimization)", fontsize=12)
plt.axhline(y=10, color='red', linestyle='--', alpha=0.5, label='Critical Threshold')
plt.legend()

plt.savefig(OUTPUT_FILE, dpi=300)
print(f"--- RESULT GENERATED: {OUTPUT_FILE} ---")
