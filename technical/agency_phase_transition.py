...
from scipy.ndimage import gaussian_filter from multiprocessing import
Pool import time

# --- HIGH FIDELITY CONFIGURATION (M4 MAX MODE) ---

GRID_SIZE = 100 NUM_AGENTS = 50 STEPS = 300 \# Increased steps for
longer trajectories TRIALS_PER_PARAM = 100 \# Increased from 20 to 100
(smoother data) RESOLUTION = 50 \# Increased from 20 to 50 (higher pixel
density)

BIAS_RANGE = np.linspace(0, 1.0, RESOLUTION)\
NOISE_RANGE = np.linspace(0.1, 1.0, RESOLUTION)

def run_simulation_step(args): """ Simulates a single agent trajectory
and calculates Agency Metrics. """ bias_strength, noise_level = args
target = np.array(\[GRID_SIZE // 2, GRID_SIZE // 2\]) pos =
np.array(\[np.random.randint(0, GRID_SIZE), np.random.randint(0,
GRID_SIZE)\], dtype=float) trajectory = \[pos.copy()\]
...
def generate_heatmap(): print(f"--- INITIATING HIGH-FIDELITY SIMULATION
---") print(f"Resolution: {RESOLUTION}x{RESOLUTION} \| Trials:
{TRIALS_PER_PARAM}") print(f"Total Simulations: {RESOLUTION \*
RESOLUTION \* TRIALS_PER_PARAM}")
